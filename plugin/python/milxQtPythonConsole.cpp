...
void milxQtPythonConsole::stdOut(const QString& s)
{
    _stdOut += s;
    int idx;
...
void milxQtPythonConsole::stdErr(const QString& s)
{
    _hadError = true;
    _stdErr += s;
    int idx;
...
milxQtPythonConsole::~milxQtPythonConsole()
{
    writeSettings();
}
...
void milxQtPythonConsole::appendCommandPrompt(bool storeOnly)
{
    if (storeOnly)
    {
        _commandPrompt = "...> ";
...
void milxQtPythonConsole::setCurrentFont(const QColor& color, bool bold)
{
...
int milxQtPythonConsole::commandPromptPosition()
{
...
void milxQtPythonConsole::insertCompletion(const QString& completion)
{
...
//-----------------------------------------------------------------------------
void milxQtPythonConsole::handleTabCompletion()
{
    QTextCursor textCursor   = this->textCursor();
...
void milxQtPythonConsole::keyPressEvent(QKeyEvent* event)
{
...
void milxQtPythonConsole::changeHistory()
{
...
void milxQtPythonConsole::readSettings()
{
    ...
    _history = settings.value("history").toStringList(); ///Restore history
...
void milxQtPythonConsole::writeSettings()
{
    ...
    QStringList savedHistory = settings.value("history").toStringList();
    savedHistory.removeDuplicates();
    _history.removeDuplicates();
    savedHistory << _history;
    for(int j = 0; j < savedHistory.size()-maxHistorySize; j ++) ///Trim list greater than 100
        savedHistory.removeFirst();
    settings.setValue("history", savedHistory);
    settings.endGroup();
}

//-----------------------------------------------------------------------------

void milxQtPythonConsole::consoleMessage(const QString & message)
{
...
void milxQtPythonConsole::createActions()
{
...
void milxQtPythonConsole::createConnections()
{
    //Actions
