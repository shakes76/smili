...
milxQtRegistrationNifti::milxQtRegistrationNifti(QObject * parent) : QObject(parent)
{
	return;
}

void milxQtRegistrationNifti::cpp2def_async(ParamsF3D params)
{
...
void milxQtRegistrationNifti::average_async(QString outputName, QStringList filenames)
{
	future = QtConcurrent::run(this, &milxQtRegistrationNifti::average, outputName, filenames);
}

int milxQtRegistrationNifti::average(QString outputName, QStringList filenames)
{
	if (filenames.size() < 2) { emit averageCompleted(); return EXIT_FAILURE; }
	...
	for (int i = 0; i < nbargs; i++)
	{
		...
		QString path;
		if (i == 0) { path = ""; }
		else if (i == 1) {	path = outputName; }
		else { path = filenames[i - 2]; }
		...
		n.find(".img.gz") != std::string::npos)
	{
		...
		if (tempImage == NULL){
			...
			for (int i = 0; i < nbargs; i++){ free(arguments[i]); }
			free(arguments);
...
int milxQtRegistrationNifti::cpp2def(ParamsF3D params)
{
	/* Read the reference image */
	nifti_image *referenceImage = reg_io_ReadImageHeader(params.referenceName);
	...
	nifti_image *controlPointImage = reg_io_ReadImageFile(params.outputControlPointGridName);
	...
	reg_io_WriteImageFile(deformationFieldImage, params.defOutputName);
