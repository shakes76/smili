...
//Image output related
typedef unsigned char charPixelType;
typedef itk::Image<charPixelType, milx::imgDimension> charImageType;

//Supported operations
enum operations {none = 0, convert, duplicate, cat, split, scale, decimate, smooth, laplacian, thresholdscalars, flip, diffscalars, copyscalars, diffscalarspairs, statscalars, removescalars, mse, procrustes, icp, clip,
                voxelise, orient, flat};

/**
  \file milxDeformableModelApp.cxx
  \brief A swiss-army knife of deformable model/surface processing. This application implements single/batch processing of models with various processing options found in the SMILI library.
...
int main(int argc, char *argv[])
{
  ...
  //---------------------------
  ///Process Arguments
  CmdLine cmd("A diagnostic tool for deformable models/surface operations", ' ', milx::NumberToString(milx::Version));
...
//  std::vector<std::string> altfilenames = altmultinames.getValue();
  ...
  std::string outputFormatName = outputFormatArg.getValue();
  ...
  if(voxeliseArg.isSet())
  {
    if(!prefixArg.isSet())
    {
      milx::PrintError("Voxelise Argument Error: Output Prefix (-p) must be provided.");
      milx::PrintError("Re-run with the prefix name set.");
      exit(EXIT_FAILURE);
    }
    if(!outputFormatArg.isSet())
    {
      milx::PrintWarning("No output format set. Using Nifty format (nii.gz) by default.");
      outputFormatName = "nii.gz";
    }
    //voxel
    milx::PrintInfo("Voxelising surfaces: ");
    operation = voxelise;
  }
  if(orientArg.isSet())
  {
  ...
  itk::SmartPointer<charImageType> refImage;
  if(orientArg.isSet())
  {
    std::cerr << "Reading Reference Image... "; //dont need to read data just header
    if( !milx::File::OpenImage<charImageType>(orientName, refImage) ) //Error printed inside
      exit(EXIT_FAILURE);
    std::cerr << "Done" << std::endl;
  }
  ...
  switch(operation)
  {
    ...
    case voxelise:
      Model.VoxeliseCollection<charImageType>(collection, voxelSpacing, imgCollection);
    ...
    case orient:
      Model.ApplyOrientationCollection<charImageType>(collection, refImage);
  ...
  if(multiOutputRequired)
  {
    if(collection->GetNumberOfItems() > 0)
    {
      ...
      {
        std::string ext;
        if (outputFormatArg.isSet() || imageOutputRequired) {
          ext = outputFormatName;
        } else {
      ...
      if(imageOutputRequired)
        milx::File::SaveImages<charImageType>(names, imgCollection);
      else
